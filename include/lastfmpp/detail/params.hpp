...
namespace lastfmpp::detail {
    ...
    struct _to_string {
        ...
        inline std::string operator()(std::experimental::string_view s) const {
            return s.to_string();
        }
    ...
    struct _bifunctor {
        template <typename PairT, typename F, typename G> constexpr auto operator()(PairT&& pair, F&& f, G&& g) const {
    ...
    struct _bifunctor_first {
        template <typename PairT, typename F> constexpr auto operator()(PairT&& pair, F&& f) const {
    ...
    struct _make_params {
      ...
      private:
        ...
        static std::tuple<std::string, std::experimental::optional<std::string>> make_param(PairT&& t) {
            return hana::unpack(std::forward<PairT>(t), [](auto&& a, auto&& b) {
                return std::make_tuple(
                    std::string{a}, hana::transform(hana::flatten(std::experimental::make_optional(b)), _to_string{}));
            });
        }
    };
    ...
    struct _sv_impl {
        ...
        static std::experimental::basic_string_view<CharT, CharTraits> view(std::basic_string<CharT, CharTraits>&& s) {
            static_assert(std::is_void<decltype(s)>::value,
                          "Cannot create basic_string_view from an rvalue basic_string");
        }

        template <typename CharT, typename CharTraits>
        static std::experimental::basic_string_view<CharT, CharTraits>
        view(const std::basic_string<CharT, CharTraits>&& s) {
            static_assert(std::is_void<decltype(s)>::value,
                          "Cannot create basic_string_view from an rvalue basic_string");
        }

        template <template <typename, typename> class S, typename CharT, typename CharTraits>
        static std::experimental::basic_string_view<CharT, CharTraits> view(S<CharT, CharTraits>&& s) {
            return s;
        }

        template <typename CharT> static std::experimental::basic_string_view<CharT> view(const CharT* s) {
            return s;
        }
    };
...
#define SV(s) ::lastfmpp::detail::_sv_impl::view(s)
    ...
    constexpr auto only_when =
        hana::flip(hana::lockstep(hana::filter)(hana::lift<hana::ext::std::experimental::Optional>, hana::id));
